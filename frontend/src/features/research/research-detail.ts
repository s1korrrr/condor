...
export function metricValue(input: unknown): string {
  ...
  if (metric.unit === "fraction")
    return `${(number * 100).toLocaleString(undefined, { maximumFractionDigits: 4 })}%`;
  const unit =
    typeof metric.unit === "string" && metric.unit ? ` ${metric.unit}` : "";
...
export function archiveMetricsAvailable(value: unknown): boolean {
  return (
    !!value &&
    typeof value === "object" &&
    (value as Record<string, unknown>).metrics_state ===
      "SOURCE_HASH_MATCHED; NOT_ECONOMIC_ADJUDICATION"
  );
}
...
export function documentPath(
  ...
  if (!["node", "archive", "receipt"].includes(scope))
    throw new Error("Invalid document scope");
  if (!server.trim() || !id.trim() || !ref.trim())
...
export function isolatedDocument(source: string, fragment = ""): string {
  const policy =
    "default-src 'none'; frame-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:; font-src data:; connect-src 'none'; form-action 'none'; object-src 'none'; base-uri 'none'";
  let anchor = fragment.replace(/^#/, "");
  try {
    anchor = decodeURIComponent(anchor);
  } catch {
    /* Preserve literal legacy IDs. */
  }
  const target = JSON.stringify(anchor).replaceAll("<", "\\u003c");
  const navigation = anchor
    ? `<script>addEventListener('load',()=>{const id=${target};(document.getElementById(id)||document.getElementsByName(id)[0])?.scrollIntoView();},{once:true})</script>`
    : "";
  const encoded = (source + navigation)
    .replaceAll("&", "&amp;")
    .replaceAll('"', "&quot;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;");
...
export function researchLabel(key: string) {
