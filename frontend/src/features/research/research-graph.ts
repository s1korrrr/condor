...
export function layoutResearchGraph(
  ...
  if (!Array.isArray(data.nodes) || !Array.isArray(data.edges))
    throw new Error("Research graph is unavailable");
  ...
  for (let index = 1; index < nodes.length; index++) {
    ...
    const count = Math.min(
      ring === 1 ? 8 : ring === 2 ? 16 : 25,
      nodes.length - first,
    );
    const angle =
      -Math.PI / 3 +
      ((index - first) / count) * Math.PI * 2 +
      (ring % 2 ? 0 : 0.14);
    ...
    edges.push({
      id:
        typeof edge.id === "string"
          ? edge.id
          : `${edge.source}:${edge.target}:${edge.relation}:${index}`,
      source: edge.source,
      target: edge.target,
      relation: edge.relation,
      basis: typeof edge.basis === "string" ? edge.basis : "UNAVAILABLE",
    });
  }
  return {
    nodes,
    edges,
    truncated: data.truncated === true || data.nodes.length > 50,
    omittedEdges,
  };
}
