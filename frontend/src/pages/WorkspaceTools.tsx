...
export function WorkspaceTools() {
  return <Navigate to="/settings?tab=tools" replace />;
}

export function WorkspaceCapabilities() {
  const {access,unavailableReason,refetch}=useServerCapabilities();
  return <div className="quant-workspace"><header className="quant-heading"><div><h2>Server capabilities</h2><p>Connections, execution services and automation.</p></div><button onClick={()=>void refetch()}>Refresh status</button></header>
    {unavailableReason?<div role="alert" className="quant-notice">{unavailableReason}</div>:null}
    <div className="quant-tools-list"><section className="quant-panel"><header className="quant-panel-heading"><h2>Execution & automation</h2></header>
