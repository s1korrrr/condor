...
def _validate_config_against_template(
    ...
    for param_name, param_info in template.items():
        ...
        if required is None:
            required = param_info.get("default") is None
        if required and param_name not in config_data:
...
async def modify_controllers(
    ...
    if target == "controller":
        from condor.rsi_controllers import is_managed_rsi_controller

        if controller_name and is_managed_rsi_controller(controller_name):
            raise ValueError(
        ...
        if action == "upsert":
            ...
            exists = config_name in [
                c.get("_config_name") or c.get("config_base_name") or c.get("id")
